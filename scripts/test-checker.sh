...
# ============================================================================
cp "$manifest" "$manifest.bak"
edit "$manifest" 's/^  methodology_version: 1.0.2/  methodology_version: 0.4.0-verification-first/'
cat >> "$manifest" <<'EOF'

migration:
  mode: explicit_version_bound_migration
  source_methodology_version: 0.4.0-verification-first
  target_methodology_version: 1.0.2
...
cp "$manifest" "$manifest.bak"
cp "$gate_log" "$gate_log.bak"
edit "$manifest" 's/^  methodology_version: 0.4.0-verification-first/  methodology_version: 1.0.2/'
edit "$gate_log" 's/^schema_version: 2/schema_version: 1/'
run_checker "$proj"
if printf '%s\n' "$CHECK_OUT" | grep -q 'declares methodology_version 1.0.2' &&
